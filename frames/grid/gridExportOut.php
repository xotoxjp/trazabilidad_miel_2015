...
function getWhereClause($col, $oper, $val){
    ...
    return " WHERE $col {$ops[$oper]} '$val' AND export.estado='SALIDA' ";
}


$where = " WHERE export.estado='SALIDA' "; //if there is no search request sent by jqgrid, $where should be empty
    ...
    foreach ($jsonIterator as $key => $val) {
        if(is_array($val)) {
            ...
            if($key=='field'){
                ...
                if($val=='num_loteGSB') $field='export.num_loteGSB';
...
$result = mysql_query("SELECT COUNT(*) AS count FROM export WHERE export.estado='SALIDA' ");
...
stock.rango , clientes.razon_social, provedores.c1 as renapa, laboratorio.tipo_miel, laboratorio.color, deposito.peso, deposito.tara, stock.peso_neto, 
export.fecha_export, deposito.remito, deposito.almacen, almacenes.razon_social as sala_ext
...
".$where." ORDER BY ".$sidx." ".$sord." LIMIT ".$start." , ".$limit." "; 
 ...
 while($row = mysql_fetch_array($result,MYSQL_ASSOC)) { 
    ...
    ,$row['peso'],$row['tara'],$row['peso_neto'],$row['color'],$row['fecha_export'],$row['tipo_miel'],$row['num_loteGSB'],$row['rango'],""); 
    $i++;
 } 
