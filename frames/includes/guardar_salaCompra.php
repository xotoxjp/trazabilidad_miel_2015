...
if($accion=="edit"){        
    $SQL = 'UPDATE almacenes 
            SET razon_social="'.$razon.'", dir1 ="'.$dir1.'",Localidad="'.$localidad.'",cod_postal="'.$cod_postal.'", provincia="'.$provincia.'", pais="'.$pais.'", contacto="'.$contacto.'",tel="'.$tel.'", cel="'.$cel.'", fax="'.$fax.'", nextel = "'.$nextel.'", email="'.$email.'",contacto1="'.$contacto1.'",
            tel1="'.$tel1.'",cel1="'.$cel1.'", email1="'.$email1.'",hab_senasa="'.$senasa.'"
            WHERE almacen_id = "'.$almacen_id.'"';
}
else{
    $SQL = "INSERT INTO almacenes (almacen_id,razon_social,dir1, Localidad, cod_postal, provincia, pais,  contacto, tel, cel, fax, nextel, email, contacto1, tel1, cel1, email1,tipo_almacen, hab_senasa)  
            VALUES ('$almacen_id','$razon','$dir1','$localidad','$cod_postal','$provincia','$pais','$contacto','$tel','$cel','$fax','$nextel','$email', '$contacto1', '$tel1','$cel1','$email1','$tipo_almacen','$senasa')";
}
echo $SQL;    
